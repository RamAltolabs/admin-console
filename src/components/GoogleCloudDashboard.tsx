...
const GoogleCloudConsoleDashboard: React.FC = () => {
  ...
  const theme = useMemo(() => {
    ...
    const nebula = {
      shellBg: 'linear-gradient(180deg, #f5fffb 0%, #eefdf7 100%)',
      panel: '#ffffff',
      panelSoft: '#effdf7',
      panelSofter: '#f7fffb',
      border: '#d1fae5',
    ...
    if (selectedProject === 'Pluto') return pluto;
    if (selectedProject === 'Nebula') return nebula;
    return earth;
  }, [selectedProject]);
  ...
  const nebulaRegion = process.env.REACT_APP_GCP_REGION_NEBULA || process.env.REACT_APP_GCP_NEBULA_REGION || 'us-east1';
  const displayRegion =
    selectedProject === 'Earth'
      ? earthRegion
      : selectedProject === 'Pluto'
        ? plutoRegion
        : selectedProject === 'Nebula'
          ? nebulaRegion
        : (overview?.region || '-');
  ...
  const projectOptions = useMemo(() => {
    const base = [overview?.projectName, 'Earth', 'Pluto', 'Nebula'].filter(Boolean) as string[];
        ...
        <div className="rounded-3xl border p-8" style={{ background: theme.panel, borderColor: theme.border }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
            {[
              ...
              { id: 'Nebula', desc: 'Core platform observability, network and service stability view.' }
            ].map((p) => (
              <button
                key={p.id}
